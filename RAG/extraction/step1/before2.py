...
def is_likely_table(text):
    ...
    # Tambahan: Cek jika teks mengandung pola header dan data yang konsisten
    if len(lines) > 1:
        header = lines[0].split()
        data = lines[1].split()
        if len(header) == len(data) and len(header) > 1:
            return True
...
def validate_table(table_content):
    ...
    # Tambahan: Cek jika ada pola header dan data yang konsisten
...
if __name__ == "__main__":
    filename = "studi_kasus/4_Tabel_Satu_Halaman_Normal_V2.pdf"  # Ganti dengan nama file PDF Anda
